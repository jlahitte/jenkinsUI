...
(function() {
  ...
  Repo = (function() {
    ...
    Repo.prototype.compareCommits = function(baseCommit,headCommit, callback,baseEnv,headEnv) {
      return this.client.get("/repos/" + this.name + "/compare/" + baseCommit+"..."+headCommit, function(err, status, body, header) {
        if (err) {
          return callback(err);
        }
        if (status !== 200) {
          return callback(new Error("Repo compare error"));
        } else {
          return callback(null, body, header,baseEnv,headEnv);
        }
      });
    };
